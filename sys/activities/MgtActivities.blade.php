...
<div class="card-body pt-3 bg-light shadow-lg table-responsive">
    {!! Alert(
        $icon = 'fa-info',
        $class = 'alert-primary',
        $Title = 'Let\'s manage project activities',
        $Msg = 'Add, remove and edit the project activity inventory',
    ) !!}
</div>
<div class="card-body pt-3 bg-light shadow-lg table-responsive">
        ...
        <tbody>
            @isset($Activities)
                ...
                <td>Q{{ $data->FinancialQuater }}</td> --}}
                        ...
                        <td
                            class="@switch(strtolower($data->ProgressStatus))
                            @case(strtolower('Ongoing'))
                                bg-warning fw-bolder text-dark
                                @break
                            @case(strtolower('Pending'))
                                bg-info fw-bolder text-light
                                @break
                            @case(strtolower('Completed'))
                                bg-success fw-bolder text-light
                                @break
                            @case(strtolower('Terminated'))
                                bg-danger fw-bolder text-light
                                @break
                            @default
                                bg-light fw-bolder text-dark
                        @endswitch">
                            {{ $data->ProgressStatus }}
                        </td>
...
{{ DescModal($Activities, $Title = 'View the objectives  attached to selected activity ', $ModalID = 'ViewDesc', $col = 'OutPuts') }}
    ...
    @foreach ($Activities as $up)
        ...
        <form action="{{ route('MassUpdate') }}" class="" method="POST">
            ...
            <div class="row">
                ...
                <input type="hidden" name="TableName" value="project_activities">

                {{ RunUpdateModalFinal($ModalID = $up->id, $Extra = '', $csrf = null, $Title = null, $RecordID = $up->id, $col = '4', $te = '12', $TableName = 'project_activities') }}
            </div>
